...
if [ ! -f "$HOME_AGENT/config.json" ]; then cat > "$HOME_AGENT/config.json" <<'JSON'
{"GROQ_API_KEY":"","MODELS":["openai/gpt-oss-120b","llama-3.3-70b-versatile","llama-3.1-8b-instant"],"TELEGRAM_BOT_TOKEN":"","TELEGRAM_ADMIN_ID":""}
JSON
fi
cat > "$PREFIX/bin/hy" <<EOF
#!/data/data/com.termux/files/usr/bin/bash
exec python "$HOME_AGENT/auto_agent.py" "\$@"
EOF
cat > "$PREFIX/bin/hud" <<EOF
#!/data/data/com.termux/files/usr/bin/bash
exec bash "$HOME_AGENT/start_hud.sh" "\$@"
EOF
chmod +x "$PREFIX/bin/hy" "$PREFIX/bin/hud"
echo '[+] Installed: hy and hud'
